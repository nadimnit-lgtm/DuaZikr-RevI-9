...
class MainActivity : AppCompatActivity() {
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        if (this::webView.isInitialized && event.keyCode != KeyEvent.KEYCODE_BACK) {
            ...
            // TV-only remote bridge for real Android TV hardware.
            // Mobile and tablet keep the normal WebView/touch path unchanged.
            if (isTelevision() && event.action == KeyEvent.ACTION_DOWN) {
                ...
                if (jsKey != null) {
                    webView.evaluateJavascript(
                        "(window.__azkarTvKey && window.__azkarTvKey('" + jsKey + "')) ? 'true' : 'false'",
                        null
                    )
                    return true
                }
